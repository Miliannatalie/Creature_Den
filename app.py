...
app.config["DISCORD_CLIENT_ID"] = 1090341140974284842    # Discord client ID.
app.config["DISCORD_CLIENT_SECRET"] = "NuK4fasK_lS_nnFzRDCXqdqc-z0J4Ibq"                # Discord client secret.
app.config["DISCORD_REDIRECT_URI"] = "http://127.0.0.1:5000/callback"                 # URL to your callback endpoint.
app.config["DISCORD_BOT_TOKEN"] = "MTA5MDM0MTE0MDk3NDI4NDg0Mg.GDKoTT.cmQm6JFEJl_RsOy0_-WQVyrK-KkPGe44kd_sAY"# Required to access BOT resources.
...
@app.route("/login")
def login():
    return discord.create_session(data=dict(next=request.args.get("next")), scope=["identify"], permissions=8)
...
def ability():
    ...
    return render_template("ability_info.html", current_ability=current_ability, all_creatures=creatures_with_ability)
